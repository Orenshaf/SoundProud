...
const CommentIndex = ({comments, currentUserId, newComments}) => {
    const trackComments = comments.reverse().map(comment => {
    ...
    const newTrackComments = newComments.map((comment, i) => {
        return <CommentItem key={i} trackTime={comment.track_time} body={comment.body} createdAt={"less than one hour ago"} userId={comment.user_id} currentUserId={currentUserId}/>
    })
    return (
